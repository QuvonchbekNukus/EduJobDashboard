...
@section('content')
    <div class="modern-shell">
        <div class="modern-hero mb-4">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                <div>
                    ...
                    <h1 class="modern-title">Seeker tahrirlash</h1>
                    <p class="text-muted mb-0">Nomzod ma`lumotlarini yangilang va saqlang.</p>
                </div>
        ...
        <div class="modern-card p-4 p-md-5">
            ...
            <form method="POST" action="{{ route('seekers.update', $seeker) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    >
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $seeker->user_id) == $user->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    >
                        ...
                        @foreach ($regions as $region)
                            <option value="{{ $region->id }}" {{ old('region_id', $seeker->region_id) == $region->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    >
                        <option value="" disabled>Seeker turini tanlang</option>
                        @foreach ($seekersTypes as $seekersType)
                            <option value="{{ $seekersType->id }}" {{ old('seekertype_id', $seeker->seekertype_id) == $seekersType->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    >
                        <option value="" {{ old('subject_id', $seeker->subject_id) ? '' : 'selected' }}>Tanlanmagan</option>
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}" {{ old('subject_id', $seeker->subject_id) == $subject->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    >
                        <option value="" {{ old('work_format', $seeker->work_format) ? '' : 'selected' }}>Tanlang</option>
                        <option value="online" {{ old('work_format', $seeker->work_format) === 'online' ? 'selected' : '' }}>Online</option>
                        <option value="offline" {{ old('work_format', $seeker->work_format) === 'offline' ? 'selected' : '' }}>Offline</option>
                        <option value="gibrid" {{ old('work_format', $seeker->work_format) === 'gibrid' ? 'selected' : '' }}>Gibrid</option>
                    </select>
                    @error('work_format')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
